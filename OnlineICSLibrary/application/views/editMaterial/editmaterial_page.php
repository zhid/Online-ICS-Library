		...
		<script>
			...
			$(function() {
				$('#success_button').click(function () {
					...
					window.location = '<?php $this->load->helper('url'); echo base_url();?>index.php/editMaterial/editmaterial';
				});
			});
			...
			function editmaterial()
			{
				...
				$.ajax({
					...
					success: function(response)
					{
						$("#user_list").html(response);
						$('#mylibrary_box').height($('#user_list').height()+70);
						$(".editmaterial_button").on("click", editmaterial);
						$("#editmaterial_form").on("submit", submitEditedMaterial);
					}
				});
				return false;
			}
			
			function submitEditedMaterial()
			{
				var action = $("#editmaterial_form").attr('action');
					
				$.ajax({
					type: "POST",
					url: action,
					data: $("#editmaterial_form").serialize(),
					success: function(response)
					{
						if(response == "edited")
						{
	...
	<body>
		...
		<div id="success_box" style="display:none;"> 
			<div id="success_message">
				<h1>MATERIAL SUCCESSFULLY EDITED</h1>
			...
			<div id="mylibrary_content">
				...
				<div id="manageuser_container">				
					<form id ="edit_form" name ="edit_form" action="<?php $this->load->helper('url'); echo base_url();?>index.php/editMaterial/editmaterial/editMaterials" method="POST">
						<label>search: </label>
						<input type = "text" placeholder = "type a keyword here" name = "edit_keyword" id = "edit_keyword" title = "must be alphanumeric only" pattern = "[a-zA-Z0-9]+" required focus/>
