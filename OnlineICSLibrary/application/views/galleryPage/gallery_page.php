	...
	<head>
		...
		<script>
			$(document).ready(function() {
				$('#gallery_elements').height($('.material_container').length * 125);
				$('#gallery_container').height($('#gallery_elements').height() + 75);
			...
			$(function() {
				$('#material_section').change(function() {
					...
					$.ajax({
						type: "POST",
						url: '<?php $this->load->helper('url'); echo base_url();?>index.php/galleryPage/gallery/loadMaterials/0',
						data: form_data,
						success: function(response)
						{
							$("#gallery_elements").html(response);
							$('#gallery_elements').height($('.material_container').length * 125);
							$('#gallery_container').height($('#gallery_elements').height() + 75);
						}
					});
		...
		<div id="gallery_container">
			<div id="gallery_header">
				...
				<div id="header_control">
					<select id="material_section" name="material_section">
			...
			<div id="gallery_elements">	
				<!--HERE IS FOR MATERIAL LOADING-->
			</div>
		</div>	
